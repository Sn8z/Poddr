...
class PoddrCollectionSelectionDialog extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return PoddrDialog(
      ...
      child: StreamBuilder<List<PodcastCollection>>(
            ...
            builder: (context, subSnapshot) {
              ...
              return Column(
                children: allCollections.map((collection) {
                  final isSelected =
                      currentCollections.any((c) => c.id == collection.id);
                  return PoddrToggle(
                    ...
                    onChanged: (value) {
                      if (value) {
                        collectionsProvider.addSubscriptionToCollection(
                            subscriptionId, collection.id);
                      } else {
                        collectionsProvider.removeSubscriptionFromCollection(
                            subscriptionId, collection.id);
                      }
                    },
                  );
