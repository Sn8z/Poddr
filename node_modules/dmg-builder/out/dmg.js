...
class DmgTarget extends _electronBuilderLib().Target {
  ...
  async signDmg(artifactPath) {
    ...
    if (!(await (0, _builderUtil().isCanSignDmg)())) {
    ...
    let identity = await (0, _codeSign().findIdentity)(certificateType, qualifier, keychainName);

    if (identity == null) {
      identity = await (0, _codeSign().findIdentity)("Mac Developer", qualifier, keychainName);
