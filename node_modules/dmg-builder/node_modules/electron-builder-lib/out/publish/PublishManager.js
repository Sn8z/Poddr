...
class PublishManager {
  ...
  async artifactCreated(event) {
    ...
    if (event.isWriteUpdateInfo && target != null && eventFile != null && !this.cancellationToken.cancelled && (packager.platform !== _index().Platform.WINDOWS || isSuitableWindowsTarget(target))) {
...
async function getPublishConfigsForUpdateInfo(packager, publishConfigs, arch) {
  ...
  if (publishConfigs.length === 0) {
    ...
    if (repositoryInfo != null && repositoryInfo.type === "github") {
      const resolvedPublishConfig = await getResolvedPublishConfig(packager, {
...
async function getPublishConfigs(packager, targetSpecificOptions, arch, errorIfCannot) {
  ...
  if (publishers == null) {
    ...
    if (serviceName != null) {
      ...
      return [await getResolvedPublishConfig(packager, {
  ...
  return await _bluebirdLst().default.map((0, _builderUtil().asArray)(publishers), it => getResolvedPublishConfig(packager, typeof it === "string" ? {
    provider: it
...
async function getResolvedPublishConfig(packager, options, arch, errorIfCannot) {
