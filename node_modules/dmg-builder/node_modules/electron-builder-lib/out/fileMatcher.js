  ...
  return _bluebirdLst().default.map(matchers, async matcher => {
    ...
    if (fromStat.isFile()) {
      ...
      if (toStat != null && toStat.isDirectory()) {
        return await (0, _fs().copyOrLinkFile)(matcher.from, path.join(matcher.to, path.basename(matcher.from)), fromStat);
      }

      await (0, _fsExtraP().mkdirs)(path.dirname(matcher.to));
