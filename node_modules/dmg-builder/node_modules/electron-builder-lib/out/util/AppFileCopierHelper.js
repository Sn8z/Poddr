...
exports.ELECTRON_COMPILE_SHIM_FILENAME = exports.NODE_MODULES_PATTERN = void 0;
...
async function computeFileSets(matchers, transformer, packager, isElectronCompile) {
  const fileSets = [];
  ...
  for (const matcher of matchers) {
    ...
    if (!isHoistedNodeModuleChecked && matcher.from === packager.appDir && !fileWalker.isNodeModulesHandled) {
      ...
      if ((await (0, _fs().statOrNull)(path.join(packager.appDir, "node_modules"))) == null) {
        // in the prepacked mode no package.json
        const packageJsonStat = await (0, _fs().statOrNull)(path.join(packager.appDir, "package.json"));
    ...
    await _bluebirdLst().default.filter(files, (it, index) => {
...
async function copyHoistedNodeModules(packager, mainMatcher) {
  ...
  return await _bluebirdLst().default.mapSeries(rootPathToCopier.keys(), async source => {
...
async function compileUsingElectronCompile(mainFileSet, packager) {
  ...
  await _bluebirdLst().default.map(mainFileSet.files, file => {
    if (file.includes(NODE_MODULES_PATTERN) || file.includes(BOWER_COMPONENTS_PATTERN) || !file.includes(path.sep, nextSlashIndex) // ignore not root files
