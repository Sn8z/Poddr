...
class AsarPackager {
  ...
  async createPackageFromFiles(fileSet, packager) {
    ...
    if (this.options.smartUnpack !== false) {
      await (0, _unpackDetector().detectUnpackedDirs)(fileSet, unpackedDirs, unpackedDest, rootForAppFilesWithoutAsar);
    }
    ...
    const correctDirNodeUnpackedFlag = async (filePathInArchive, dirNode) => {
      for (const dir of unpackedDirs) {
        if (filePathInArchive.length > dir.length + 2 && filePathInArchive[dir.length] === path.sep && filePathInArchive.startsWith(dir)) {
          ...
          await (0, _fsExtraP().ensureDir)(path.join(unpackedDest, filePathInArchive));
          break;
        }
      }
    };
    ...
    for (let i = 0, n = fileSet.files.length; i < n; i++) {
      ...
      if (stat.isSymbolicLink()) {
        this.fs.getOrCreateNode(pathInArchive).link = stat.relativeLink;
      ...
      if (isUnpacked) {
        if (!dirNode.unpacked && !dirToCreateForUnpackedFiles.has(fileParent)) {
          ...
          await (0, _fsExtraP().ensureDir)(path.join(unpackedDest, fileParent));
        }
