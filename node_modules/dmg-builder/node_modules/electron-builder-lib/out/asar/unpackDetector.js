...
async function detectUnpackedDirs(fileSet, autoUnpackDirs, unpackedDest, rootForAppFilesWithoutAsar) {
  ...
  for (let i = 0, n = fileSet.files.length; i < n; i++) {
    ...
    const index = file.lastIndexOf(_AppFileCopierHelper().NODE_MODULES_PATTERN);

    if (index < 0) {
      continue;
    }

    let nextSlashIndex = file.indexOf(path.sep, index + _AppFileCopierHelper().NODE_MODULES_PATTERN.length + 1);

    if (nextSlashIndex < 0) {
      continue;
    }

    if (file[index + _AppFileCopierHelper().NODE_MODULES_PATTERN.length] === "@") {
