...
const bundledCertKeychainAdded = new (_lazyVal().Lazy)(async () => {
  ...
  if (!list.includes(keychainPath)) {
    await (0, _builderUtil().exec)("security", ["list-keychains", "-d", "user", "-s", keychainPath].concat(list));
  }
});

function getCacheDirectory() {
  const env = process.env.ELECTRON_BUILDER_CACHE;
  return (0, _builderUtil().isEmptyOrSpaces)(env) ? path.join((0, _os().homedir)(), "Library", "Caches", "electron-builder") : env;
}
...
async function removeKeychain(keychainFile) {
  try {
    await (0, _builderUtil().exec)("security", ["delete-keychain", keychainFile]);
  } catch (e) {
...
async function importCerts(keychainName, paths, keyPasswords) {
  for (let i = 0; i < paths.length; i++) {
    ...
    await (0, _builderUtil().exec)("security", ["import", paths[i], "-k", keychainName, "-T", "/usr/bin/codesign", "-T", "/usr/bin/productbuild", "-P", password]); // https://stackoverflow.com/questions/39868578/security-codesign-in-sierra-keychain-ignores-access-control-settings-and-ui-p
    // https://github.com/electron-userland/electron-packager/issues/701#issuecomment-322315996

    if (await (0, _builderUtil().isMacOsSierra)()) {
      await (0, _builderUtil().exec)("security", ["set-key-partition-list", "-S", "apple-tool:,apple:", "-s", "-k", password, keychainName]);
    }
  }
...
async function getValidIdentities(keychain) {
  ...
  if (result == null || keychain != null) {
    ...
    result = Promise.all([(0, _builderUtil().exec)("security", addKeychain(["find-identity", "-v"])).then(it => it.trim().split("\n").filter(it => {
      ...
      const array = it[0].concat(it[1]).filter(it => !it.includes("(Missing required extension)") && !it.includes("valid identities found") && !it.includes("iPhone ") && !it.includes("com.apple.idms.appleid.prd.")).map(it => it.substring(it.indexOf(")") + 1).trim());
