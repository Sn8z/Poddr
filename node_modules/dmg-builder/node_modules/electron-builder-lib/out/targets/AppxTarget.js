...
class AppXTarget extends _core().Target {
  ...
  async build(appOutDir, arch) {
    ...
    await this.writeManifest((0, _pathManager().getTemplatePath)("appx"), manifestFile, arch, (await this.computePublisherName()), userAssets);
  ...
  async writeManifest(templatePath, outFile, arch, publisher, userAssets) {
    const appInfo = this.packager.appInfo;
    const options = this.options;
    const manifest = (await (0, _fsExtraP().readFile)(path.join(templatePath, "appxmanifest.xml"), "utf8")).replace(/\$\{([a-zA-Z0-9]+)\}/g, (match, p1) => {
      switch (p1) {
        case "publisher":
          return publisher;
