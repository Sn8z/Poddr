...
class NsisTarget extends _core().Target {
  ...
  async computeScriptAndSignUninstaller(defines, commands, installerPath, sharedHeader) {
    ...
    await (0, _builderUtil().execWine)(installerPath, []);
  ...
  async computeFinalScript(originalScript, isInstaller) {
    ...
    if (preCompressedFileExtensions != null) {
      for (const [arch, dir] of this.archs.entries()) {
        const preCompressedAssets = await (0, _fs().walk)(path.join(dir, "resources"), (file, stat) => stat.isDirectory() || preCompressedFileExtensions.some(it => file.endsWith(it)));
