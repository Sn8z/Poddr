...
async function appendBlockmap(file) {
  ...
  return JSON.parse((await (0, _builderUtil().executeAppBuilder)(["blockmap", "--input", file, "--compression", "deflate"])));
}
...
async function createBlockmap(file, target, packager, safeArtifactName) {
  ...
  const updateInfo = JSON.parse((await (0, _builderUtil().executeAppBuilder)(["blockmap", "--input", file, "--output", blockMapFile])));
