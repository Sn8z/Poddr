...
function _dirPackager() {
  const data = require("./packager/dirPackager");

  _dirPackager = function () {
    return data;
  };
...
class PlatformPackager {
  ...
  packageInDistributableFormat(appOutDir, arch, targets, taskManager) {
    taskManager.addTask(_bluebirdLst().default.map(targets, it => it.isAsyncSupported ? it.build(appOutDir, arch) : null).then(() => _bluebirdLst().default.each(targets, it => it.isAsyncSupported ? null : it.build(appOutDir, arch))));
  }
  ...
  async doPack(outDir, appOutDir, platformName, arch, platformSpecificBuildOptions, targets) {
    ...
    const macroExpander = it => this.expandMacro(it, arch == null ? null : _builderUtil().Arch[arch], {
      "/*": "{,/**/*}"
    });

    const resourcesPath = this.platform === _core().Platform.MAC ? path.join(appOutDir, this.electronDistMacOsAppName, "Contents", "Resources") : path.join(appOutDir, "resources");
    ...
    _builderUtil().log.info({
      ...
      [isElectron ? `electron` : `muon`]: isElectron ? config.electronVersion : muonVersion,
      appOutDir: _builderUtil().log.filePath(appOutDir)
    }, `packaging`);

    if (this.info.isPrepackedAppAsar) {
      await (0, _dirPackager().unpackElectron)(this, appOutDir, platformName, _builderUtil().Arch[arch], config.electronVersion);
    } else {
      await (isElectron ? (0, _dirPackager().unpackElectron)(this, appOutDir, platformName, _builderUtil().Arch[arch], config.electronVersion) : (0, _dirPackager().unpackMuon)(this, appOutDir, platformName, _builderUtil().Arch[arch], muonVersion));
    }
    ...
    taskManager.addTask((0, _fs().unlinkIfExists)(path.join(resourcesPath, "default_app.asar")));
    taskManager.addTask((0, _fs().unlinkIfExists)(path.join(appOutDir, "version")));
    taskManager.addTask(this.postInitApp(packContext));

    if (this.platform !== _core().Platform.MAC) {
      taskManager.addTask((0, _fsExtraP().rename)(path.join(appOutDir, "LICENSE"), path.join(appOutDir, "LICENSE.electron.txt")).catch(() => {}));
    }

    await taskManager.awaitTasks();
    await this.beforeCopyExtraFiles(appOutDir, asarOptions == null ? null : await (0, _integrity().computeData)(resourcesPath, asarOptions.externalAllowed ? {
      externalAllowed: true
    } : null));
    await _bluebirdLst().default.each([extraResourceMatchers, extraFileMatchers], it => (0, _fileMatcher().copyFiles)(it));
  ...
  async beforeCopyExtraFiles(appOutDir, asarIntegrity) {// empty impl
  }
  ...
  copyAppFiles(taskManager, asarOptions, resourcePath, outDir, platformSpecificBuildOptions, excludePatterns, macroExpander) {
    ...
    const _computeFileSets = matchers => {
      return (0, _AppFileCopierHelper().computeFileSets)(matchers, transformer, this.info, isElectronCompile).then(it => it.filter(it => it.files.length > 0));
    };

    if (this.info.isPrepackedAppAsar) {
      taskManager.addTask(_bluebirdLst().default.each(_computeFileSets([new (_fileMatcher().FileMatcher)(appDir, resourcePath, macroExpander)]), it => (0, _appFileCopier().copyAppFiles)(it, this.info)));
    } else if (asarOptions == null) {
      taskManager.addTask(_bluebirdLst().default.each(_computeFileSets(mainMatchers), it => (0, _appFileCopier().copyAppFiles)(it, this.info)));
    } else {
  ...
  getResourcesDir(appOutDir) {
    return this.platform === _core().Platform.MAC ? this.getMacOsResourcesDir(appOutDir) : path.join(appOutDir, "resources");
  }
  ...
  async checkFileInPackage(resourcesDir, file, messagePrefix, isAsar) {
    ...
    if (pathParsed.dir.includes(".asar")) {
      ...
      const outStat = await (0, _fs().statOrNull)(path.join(resourcesDir, "app", relativeFile));
  ...
  async sanityCheckPackage(appOutDir, isAsar) {
  ...
  async resolveIcon(sources, outputFormat) {
    ...
    const rawResult = await (0, _builderUtil().executeAppBuilder)(args);
    let result;
