...
function getConnectOptions() {
    ...
    if (caCert !== "false") {
        if (isUseLocalCert) {
            (_builderUtil || _load_builderUtil()).log.debug(null, "local certificate authority is used");
        }
...
class RemoteBuildManager {
    constructor(buildServiceEndpoint, projectInfoManager, unpackedDirectory, outDir, packager) {
        ...
        (_builderUtil || _load_builderUtil()).log.debug({ endpoint: buildServiceEndpoint }, "connect to remote build service");
                ...
                stream.on("response", function (headers) {
                    try {
                        ...
                        if (!checkStatus(status, reject)) {
                            (_builderUtil || _load_builderUtil()).log.warn(`Not critical server error: ${status}`);
                        }
                    } finally {
                        resolve();
                    }
        ...
        return new (_bluebirdLst2 || _load_bluebirdLst2()).default((resolve, reject) => {
            ...
            stream.on("response", headers => {
                ...
                stream.on("end", () => {
                    ...
                    (_builderUtil || _load_builderUtil()).log.debug({ result: JSON.stringify(result, null, 2) }, `remote builder result`);
            ...
            stream.on("response", headers => {
                ...
                const eventSource = new (_JsonStreamParser || _load_JsonStreamParser()).JsonStreamParser(data => {
                    if ((_builderUtil || _load_builderUtil()).log.isDebugEnabled) {
                        (_builderUtil || _load_builderUtil()).log.debug({ event: JSON.stringify(data, null, 2) }, "remote builder event");
                    }
                    ...
                    if (data.state != null) {
                        ...
                        (_builderUtil || _load_builderUtil()).log.info({ status: message }, "remote building");
                        return;
                    }
                    if (!("files" in data)) {
                        (_builderUtil || _load_builderUtil()).log.warn(`Unknown builder event: ${JSON.stringify(data)}`);
                        return;
                    }
                    ...
                    for (const artifact of this.files) {
                        (_builderUtil || _load_builderUtil()).log.info({ file: artifact.file }, `downloading remote build artifact`);
    ...
    downloadFile(id, artifact, resolve, reject) {
        ...
        const fileWritten = () => {
            ...
            (_builderUtil || _load_builderUtil()).log.info({ time: downloadTimer.endAndGet(), file: artifact.file }, "downloaded remote build artifact");
            (_builderUtil || _load_builderUtil()).log.debug({ file: localFile }, "saved remote build artifact");
    ...
    uploadUnpackedAppArchive(stream, zstdCompressionLevel, reject) {
        ...
        (_bluebirdLst2 || _load_bluebirdLst2()).default.all([this.projectInfoManager.infoFile.value, (0, (_tools || _load_tools()).getZstd)()]).then(results => {
            ...
            (_builderUtil || _load_builderUtil()).log.info("compressing and uploading to remote builder");
            ...
            zstdProcess.stdout.on("end", () => {
                (_builderUtil || _load_builderUtil()).log.info({ time: compressAndUploadTimer.endAndGet() }, "uploaded to remote builder");
            });
        }).catch(reject);
    }
}
