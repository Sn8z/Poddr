...
class PlatformPackager {
  ...
  doPack(outDir, appOutDir, platformName, arch, platformSpecificBuildOptions, targets) {
    ...
    return (0, _bluebirdLst().coroutine)(function* () {
      ...
      const macroExpander = it => _this2.expandMacro(it, arch == null ? null : _builderUtil().Arch[arch], {
        "/*": "{,/**/*}"
      });

      const framework = _this2.info.framework;
      ...
      const taskManager = new (_builderUtil().AsyncTaskManager)(_this2.info.cancellationToken);
      const asarOptions = yield _this2.computeAsarOptions(platformSpecificBuildOptions);
      const resourcesPath = _this2.platform === _core().Platform.MAC ? path.join(appOutDir, framework.distMacOsAppName, "Contents", "Resources") : (0, _Framework().isElectronBased)(framework) ? path.join(appOutDir, "resources") : appOutDir;

      _this2.copyAppFiles(taskManager, asarOptions, resourcesPath, path.join(resourcesPath, "app"), outDir, platformSpecificBuildOptions, excludePatterns, macroExpander);
      ...
      const beforeCopyExtraFiles = _this2.info.framework.beforeCopyExtraFiles;

      if (beforeCopyExtraFiles != null) {
        yield beforeCopyExtraFiles(_this2, appOutDir, asarOptions == null ? null : yield (0, _integrity().computeData)(resourcesPath, asarOptions.externalAllowed ? {
  ...
  computeAsarOptions(customBuildOptions) {
    ...
    return (0, _bluebirdLst().coroutine)(function* () {
      if (!(0, _Framework().isElectronBased)(_this3.info.framework)) {
        return null;
      }
      ...
      if (result === false) {
        const appAsarStat = yield (0, _fs().statOrNull)(path.join(_this3.info.appDir, "app.asar")); //noinspection ES6MissingAwait
  ...
  checkFileInPackage(resourcesDir, file, messagePrefix, isAsar) {
    ...
    return (0, _bluebirdLst().coroutine)(function* () {
      const relativeFile = path.relative(_this4.info.appDir, path.resolve(_this4.info.appDir, file));
    ...
    return (0, _bluebirdLst().coroutine)(function* () {
      ...
      if (custom === undefined) {
        ...
        if ((yield (0, _fs().statOrNull)(p)) == null) {
          p = path.resolve(_this6.projectDir, custom);

          if ((yield (0, _fs().statOrNull)(p)) == null) {
            throw new (_builderUtil().InvalidConfigurationError)(`cannot find specified resource "${custom}", nor relative to "${resourcesDir}", neither relative to project dir ("${_this6.projectDir}")`);
          }
        }
  ...
  getOrConvertIcon(format) {
    ...
    return (0, _bluebirdLst().coroutine)(function* () {
      ...
      const result = yield _this7.resolveIcon(sourceNames, format);

      if (result.length === 0) {
        const framework = _this7.info.framework;
  ...
  resolveIcon(sources, outputFormat) {
    ...
    return (0, _bluebirdLst().coroutine)(function* () {
      const args = ["icon", "--format", outputFormat, "--root", _this8.buildResourcesDir, "--root", _this8.projectDir, "--out", path.resolve(_this8.projectDir, _this8.config.directories.output, `.icon-${outputFormat}`)];
