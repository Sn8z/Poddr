...
function wrap(task) {
    return args => {
        ...
        (0, (_readConfigFile || _load_readConfigFile()).loadEnv)(_path.join(process.cwd(), "electron-builder.env")).then(() => task(args)).catch(error => {
            console.error((_chalk || _load_chalk()).default.red(error instanceof (_builderUtil || _load_builderUtil()).InvalidConfigurationError ? error.message : (error.stack || error).toString()));
            process.exitCode = 1;
        });
    };
}
