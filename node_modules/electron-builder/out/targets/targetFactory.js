...
function computeArchToTargetNamesMap(raw, options, platform) {
    ...
    const defaultArchs = raw.size === 0 ? [platform === (_core || _load_core()).Platform.MAC ? "x64" : process.arch] : Array.from(raw.keys()).map(it => (_builderUtil || _load_builderUtil()).Arch[it]);
