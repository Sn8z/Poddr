...
import themesJSON from "../../assets/themes/themes.json";
...
export class SettingsComponent implements OnInit, OnDestroy {
  ...
  setTheme = () => {
    ...
    root.style.setProperty(
      "--primary-bg-color",
      themesJSON.themes[this.theme]["primaryBackground"]
    );
    root.style.setProperty(
      "--secondary-bg-color",
      themesJSON.themes[this.theme]["secondaryBackground"]
    );
    root.style.setProperty(
      "--third-bg-color",
      themesJSON.themes[this.theme]["thirdBackground"]
    );
    root.style.setProperty(
      "--primary-text-color",
      themesJSON.themes[this.theme]["primaryTextColor"]
    );
    root.style.setProperty(
      "--secondary-text-color",
      themesJSON.themes[this.theme]["secondaryTextColor"]
    );
  ...
  importOPML = () => {
    ...
    ipcRenderer
      ...
      .then((response) => {
        log.info(
          "Settings component :: Reading OPML file " + response.filePaths[0]
        );
  ...
  private readOPML = (filepath) => {
    readFile(filepath, "utf8", (error, data) => {
      if (error) {
        log.error(
          "Settings component :: Something went wrong when trying to open " +
            filepath
        );
        log.error(error);
      } else {
        ...
        if (opmlDoc.documentElement.nodeName == "parsererror") {
          log.error(
            "Settings component :: An error occured while parsing " + filepath
          );
          ...
          Array.from(opmlDoc.getElementsByTagName("outline")).forEach(
            (opmlFav) => {
              log.info(opmlFav.getAttribute("xmlUrl"));
              this.favService.addFavourite(
                opmlFav.getAttribute("xmlUrl"),
                true
              );
            }
          );
          log.info(
            "Settings component :: Done importing favourites from OPML file"
          );
        }
      }
    });
      ...
      .then((response) => {
        ...
        writeFile(
          ...
          (error) => {
            if (error) {
              log.error(
                "Settings component :: An error occured while trying to create OPML file"
              );
              log.error(error);
            } else {
              log.info(
                "Settings component :: Done creating OPML file at " +
                  response.filePath
              );
