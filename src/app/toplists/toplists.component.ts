...
export class ToplistsComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.favSubscription = this.favService.favouriteTitles.subscribe(
      (value) => {
        this.favs = value;
      }
    );
    ...
    this.routeSubscription = this.route.paramMap.subscribe((params) => {
      this.region =
        params.get("region") || (this.store.get("region", "us") as string);
  ...
  getPodcastsNavigation = () => {
    this.router.navigate([
      "/toplists",
      { region: this.region, category: this.category },
    ]);
  };

  getPodcasts = () => {
    this.podcastService
      .getToplist(this.region, this.category, this.amount)
      .subscribe((data) => {
